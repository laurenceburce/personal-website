    ...
    if (contentType.startsWith("multipart/form-data")) {
      ...
      try {
        ...
        return NextResponse.json(
          { error: "That file isn't valid JSON — upload the file scripts/job-search-oracle-connect.mjs saved, unmodified." },
          { status: 400 }
        );
      }

      // scripts/job-search-oracle-connect.mjs saves { tenantHost,
      // capturedAt, storageState } — tenantHost is derived there from the
      // URL actually connected to, never re-typed by hand here, so there's
      // no chance of a mismatched host getting attached to the wrong
      // session.
      if (!parsed?.tenantHost || !parsed?.storageState) {
        return NextResponse.json(
          { error: "That file doesn't look like a session saved by job-search-oracle-connect.mjs (missing tenantHost/storageState)." },
          { status: 400 }
        );
      }
