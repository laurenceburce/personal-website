...
export async function evaluateAutoApply({ posting, findSettings, profile }) {
  ...
  if (!SUBMITTABLE_ATS_TYPES.has(resolvedAtsType)) {
    ...
    // non-submittable platform (SmartRecruiters, Workday, iCIMS, legacy
    // Taleo — see atsResolver.js) identically: nothing was attempted, so
    // nothing worth an application-table row, just the skip reason on the
    // posting itself.
    return {
