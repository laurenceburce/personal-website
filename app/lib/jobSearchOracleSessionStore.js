...
// for jobSearchAdapters/oracleRecruiting.js (already Playwright-touching) —
// it's the only place the real storage_state ever leaves this module.
import { appError, cleanId, cleanText, ensureJobSearchSchema, parseJsonColumn, requirePool, toJsonParam } from "./jobSearchDb.js";

// Same two domain families atsTypes.js/oracleRecruiting.js already treat as
// one platform — rejected outright here rather than silently accepted, so a
// mistaken upload (wrong file, wrong site) can't quietly attach a session to
// an unrelated host.
function cleanTenantHost(value) {
  const host = String(value || "").trim().toLowerCase();
  if (!/(^|\.)(taleo\.net|oraclecloud\.com)$/i.test(host)) {
    throw appError('Tenant host must be a taleo.net or oraclecloud.com address (got "' + host + '").');
  }
  return host;
}
...
// The only caller of this should be oracleRecruiting.js, looking up the
// session matching the current posting's applyUrl hostname. Returns null for
// no match — the adapter treats that identically to a genuine login wall.
...
// the login script, and are uploading the fresh one).
